...
<Picture 3> is the first frame of [Shot 3]: the boy at the middle of the bridge, a side view from the VIEWER'S LEFT, preparing to jump over the handrail.
...
summary:
[reference generation] A cinematic 15-second run-up sequence in a stylized angular 3D art style with thick visible brushstrokes and painterly color texture. RAIN is present in every shot. The boy walks and slowly runs across a stone bridge as the sky turns moody and the rain builds, then stops and turns toward his right (the VIEWER'S LEFT), then climbs over the handrail and leaps. ALL THREE shots appear in this order, each getting its full time. Every transition is smooth and organic, and the color grade stays consistent across all shots.

retention_analysis:
<Subject 1> (appears in [Shot 1]-[Shot 3]): fully_preserved - the boy's face, short dark hair, rose-pink collared shirt, black trousers, and BLACK school bag are retained in every shot. There is ONLY ONE character - the single boy <Subject 1> - in every shot; NO other people, NO second boy, NO duplicates, NO extra figures.
...
[Shot 1] At 00:00.000, a WIDE ANGLE establishing shot - the camera is positioned FAR AWAY, showing the boy as a small figure on the bridge with the full landscape, sky, and river visible. This is a WIDE ANGLE view (NOT a medium shot, NOT a close-up). The camera stays a FIXED wide view the whole time - it does NOT move closer, does NOT push in, does NOT cut to a close-up. ONLY ONE boy <Subject 1> is on the bridge, standing on the LEFT side of the bridge - there are NO other people, NO second boy, NO duplicates, NO extra characters. Only that ONE boy runs. The boy WALKS and SLOWLY RUNS along the bridge with a natural gait - each foot clearly LIFTS and LANDS separately, visible leg movement, one step at a time (NOT a smooth slide, NOT a glide, NOT a blur). The MAIN PURPOSE of this shot is to ESTABLISH THE CLIMATE AND MOOD - the sky gradually darkens from clear to moody overcast, RAIN BEGINS TO FALL and builds, the wind picks up, and the whole landscape and environment shifts from clear to rainy. The camera stays a fixed wide shot the entire time. About 4 seconds.
...
[Shot 3] At 00:10.000, cuts to <Picture 3>: a SIDE view from the VIEWER'S LEFT of <Subject 1> at the middle of the bridge in HEAVY RAIN - the boy is on the VIEWER'S LEFT of the frame, facing the handrail (toward the river). The boy is NOT standing still - he is SLOWLY LIFTING HIS LEG and CLIMBING over the low handrail, then JUMPING. He is not repeating the movement - he breathes in after the run. The camera is positioned on the VIEWER'S LEFT side and TRACKS the boy (it stays anchored on the VIEWER'S LEFT, it does NOT sweep from left to right). Then the boy LEAPS over the low handrail slowly - his body fully airborne, a clear dynamic leap. The bridge is TALL, so this is a real leap from height over the wide river, NOT a shallow pool. The camera follows his leap and swings around to his BACK side as he is in the air. The shot ENDS with the camera at his BACK as he is in the air, BEFORE he reaches the water - the cut happens right here, so the next clip picks up the jump. About 5 seconds.

overall_landscape:
We should see VISIBLE RAINFALL throughout the sequence after the first picture - rain is clearly falling and visible in [Shot 2] and [Shot 3].

overall_soundscape:
Clear footsteps on the stone bridge, RAIN THROUGHOUT the whole video (light rain building in [Shot 1], heavier rain in [Shot 2] and [Shot 3]), a building thunderstorm with distant thunder rumbles, splashing water on the bridge stones, then a rising tension as the boy climbs and leaps.
...
print('Updated jaisalproduction1.json node 138 prompt (v10)')
