...
shutil.copy(p, p + ".pre_wiring_fix.bak")
wf = json.load(open(p, encoding="utf-8"))
nodes = {str(n["id"]): n for n in wf["nodes"]}

# --- 1. Fix the link fields on ref_image_3 and ref_image_4 ---
mm = nodes["136"]
# Build link map: link_id -> (from_node, from_slot, to_node, to_slot)
link_map = {l[0]: l for l in wf["links"]}
# Find the link IDs that target 136.6 and 136.7
target_link = {}
for l in wf["links"]:
    if l[3] == "136":
        target_link[l[4]] = l[0]  # to_slot -> link_id

fixed = []
for idx, i in enumerate(mm["inputs"]):
    if i["name"] in ("ref_images.ref_image_3", "ref_images.ref_image_4"):
        lid = target_link.get(idx)  # idx = actual slot position in inputs array
        if lid:
            i["link"] = lid
            fixed.append(f"{i['name']} (slot {idx}) -> link {lid}")
        else:
            fixed.append(f"{i['name']} (slot {idx}) -> NO LINK FOUND")
print("Fixed link fields:", fixed)

# --- 2. Rewrite the prompt to official MiniMax H3 R2V style ---
# Connection order (ref_image_0..4) = <Picture 1>..5:
#   <Picture 1> = 1.png (wide base)      -> Shot 1
#   <Picture 2> = 2.png (mid rain)       -> Shot 2
#   <Picture 3> = 3.png (close-up drenched) -> Shot 3
#   <Picture 4> = 4.png (after rain)     -> Shot 4
#   <Picture 5> = 5.png (low-angle ending)  -> Shot 5
...
# Set the prompt on node 138
for n in wf["nodes"]:
    if n["id"] == "138":
        n["widgets_values"] = [prompt]
        print("Prompt updated on node 138")
