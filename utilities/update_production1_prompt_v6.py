...
CHARACTER COUNT (hard requirement):
There is ONLY ONE character in the entire video - the single young schoolboy <Subject 1>. There are NO other people, NO second boy, NO duplicates, NO extra figures, NO crowds, NO bystanders, NO reflections of a second person. Every shot shows exactly ONE boy and only that boy.
...
summary:
[reference generation] A cinematic 15-second run-up sequence in a stylized angular 3D art style with thick visible brushstrokes and painterly color texture. ONLY ONE boy appears the whole time (no other people, no duplicates). The boy runs slowly across a stone bridge as the sky slowly turns moody, then stops and turns toward his right (the VIEWER'S LEFT), then climbs over the handrail and JUMPS into the river. ALL THREE shots must appear in this order, each getting its full time - [Shot 1] MUST be a LONG WIDE establishing shot that shows the landscape and the climate change, [Shot 3] MUST show the boy JUMPING into the river. Every transition is smooth and organic, and the color grade stays consistent across all shots.

retention_analysis:
<Subject 1> (appears in [Shot 1]-[Shot 3]): fully_preserved - the boy's face, short dark hair, rose-pink collared shirt, black trousers, and BLACK school bag are retained in every shot. There is ALWAYS exactly ONE boy - never two, never a duplicate, never a second figure.
...
The target video is in a stylized angular 3D art style with thick visible brushstrokes, painterly color texture, matte finish, and angular geometric shapes - NOT photorealistic, NOT smooth CGI. There is ONLY ONE character - the single boy <Subject 1> - in every shot; NO other people, NO second boy, NO duplicates, NO extra figures. All transitions between shots are smooth, organic, and cinematic. The color grade is consistent across every shot. ALL THREE SHOTS MUST APPEAR IN THIS ORDER, EACH GETTING ITS FULL TIME - do not skip, compress, or drop any shot. [Shot 1] MUST be a LONG WIDE establishing shot (a hard requirement - it must NOT be cut short, NOT turned into a close-up, NOT skipped) that shows the landscape and the climate change. [Shot 3] MUST show the boy JUMPING into the river. All left/right references use the VIEWER's perspective.
[Shot 1] At 00:00.000, a LONG WIDE static shot (a HARD REQUIREMENT - the camera does NOT move closer, does NOT push in, does NOT cut to a close-up - it stays a FIXED wide view the whole time): ONLY ONE boy <Subject 1> at the start of <Subject 2>, a single small figure on the bridge - there is NO second person, NO duplicate, NO other figure anywhere in the frame. The boy runs SLOWLY and DELIBERATELY along the bridge with a REAL running gait - each foot clearly LIFTS and LANDS separately, visible leg movement, one step at a time (NOT a smooth slide, NOT a glide, NOT a blur, NOT a keyframe slide, NOT fast). The MAIN PURPOSE of this shot is to ESTABLISH THE CLIMATE CHANGE - the sky gradually darkens from clear to moody overcast, the wind picks up, and rain begins to fall, the whole landscape and environment shifting from clear to rainy. The camera stays a fixed wide shot the entire time. About 5 seconds.
...
[Shot 3] At 00:10.000, cuts to <Picture 3>: a SIDE view from the VIEWER'S LEFT of <Subject 1> at the middle of the bridge in HEAVY RAIN - the boy is on the VIEWER'S LEFT of the frame, on his right side. The camera is positioned on the VIEWER'S LEFT side and TRACKS the boy (it stays anchored on the VIEWER'S LEFT, it does NOT sweep from left to right). The boy moves QUICKLY and with PURPOSE - he crouches, plants his feet, and then JUMPS: he LEAPS over the low handrail and DIVES into the river, his body fully airborne, arms out, a clear dynamic leap (NOT just climbing, NOT just standing, NOT just walking - he MUST actually JUMP and go over the handrail into the water). The camera follows his leap and swings around to his BACK side as he is in the air. The shot ENDS with the camera at his BACK as he is in the air, mid-leap, so the next clip can pick up his jump. About 5 seconds.

overall_soundscape:
Clear running footsteps on the stone bridge, heavy rain, a building thunderstorm with distant thunder rumbles, splashing water on the bridge stones, then a rising tension as the boy climbs and a big SPLASH as he JUMPS into the river.
...
print('Updated jaisalproduction1.json node 138 prompt (v6)')
