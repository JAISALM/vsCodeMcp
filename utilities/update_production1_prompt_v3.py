...
The target video is in a stylized angular 3D art style with thick visible brushstrokes, painterly color texture, matte finish, and angular geometric shapes - NOT photorealistic, NOT smooth CGI. All transitions between shots are smooth, organic, and cinematic. The color grade is consistent across every shot. ALL THREE SHOTS MUST APPEAR IN THIS ORDER, EACH GETTING ITS FULL TIME - do not skip, compress, or drop any shot, especially [Shot 3].
[Shot 1] At 00:00.000, a LONG WIDE static shot (the camera does NOT move closer, does NOT push in - it stays a FIXED wide view the whole time): <Subject 1> at the start of <Subject 2>, a small figure on the bridge. The boy runs SLOWLY and DELIBERATELY along the bridge, each step clearly visible, one foot at a time, in a clear step-by-step rhythm (NOT rushing, NOT sliding, NOT a blur). The MAIN PURPOSE of this shot is to show the LANDSCAPE and how the SKY SLOWLY CHANGES MOOD - the sky gradually darkens from clear to moody overcast, the wind picks up, and rain begins to fall, the whole environment shifting. The camera stays a fixed wide shot the entire time. About 5 seconds.
...
print('Updated jaisalproduction1.json node 138 prompt (v3)')
